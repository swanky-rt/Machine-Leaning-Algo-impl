...
def plot_learning_curve_titanic(X_train, Y_train, X_test, Y_test, best_layers, best_lambd, learning_rate, step_size=10, epochs=1000):
    ...
    plt.title("Learning Curve (raisin Dataset)")
    plt.grid(True)
    plt.show()

def load_raisins_data_sets(filepath):
    data = pd.read_csv(filepath)
    #print(f"{data.head()}, {data.dtypes}")
...
if __name__ == "__main__":
    ...
    X, Y = load_raisins_data_sets(filepath+filename)
    ...
    # Split the dataset after shuffling
    split_idx = int(0.8 * X.shape[1])
    Y_train, Y_test = Y[:, :split_idx], Y[:, split_idx:]
    X_train, X_test = X[:, :split_idx], X[:, split_idx:]

    # learning curve, this value is for raisin data sets
    best_layers = [X.shape[0], 16, 16, 1]
    best_lambda = 0.1
    learning_rate = 0.01

    plot_learning_curve_titanic(X_train, Y_train, X_test, Y_test, best_layers, best_lambda, learning_rate)