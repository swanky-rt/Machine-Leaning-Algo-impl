...
def split_datasets(data_sets, random):
    ...
    X_train, X_test, y_train, y_test = train_test_split(X, Y, test_size=0.8, random_state=random)
...
if __name__ == "__main__":
    ...
    decision_tree = calculate_decision_tree(output_class, data, x_attributes, 5, 0)
    ...
    if np.mean(train_accuracy) > 85 and np.mean(test_accuracy) > 85:
        #setting max depth to enable the stopping criteria 3 to prevent the model to go for overfitting.

        train_accuracy = run_experiment_for_training_sets(data, 100, 5, 0)
        test_accuracy = run_experiment_for_testing_sets(data, 100, 5, 0)
