...
def plot_learning_curve_rice_grains(X_train, Y_train, X_test, Y_test, best_layers, best_lambd, learning_rate, step_size=10, epochs=1000):
    ...
    plt.title("Learning Curve (rice_grains Dataset)")
    plt.grid(True)
    plt.show()

def load_rice_grains_dataset(filepath):
...
if __name__ == "__main__":
    ...
    X, Y = load_rice_grains_dataset(filepath+filename)
    ...
    best_lambda = 0.01
    learning_rate = 0.01
    # learning curve, this value is for rice grains data sets
    best_layers = list(map(int, [X.shape[0], 16, 16, 1]))
    plot_learning_curve_rice_grains(X_train, Y_train, X_test, Y_test, best_layers, best_lambda, learning_rate)