...
def plot_learning_curve_credit_approval(X_train, Y_train, X_test, Y_test, best_layers, best_lambd, learning_rate, step_size=10, epochs=300):
    ...
    plt.title("Learning Curve (Credit Approval Dataset)")
    plt.grid(True)
    plt.show()

def load_credit_approval_dataset(filepath):
...
if __name__ == "__main__":
    ...
    X, Y = load_credit_approval_dataset(filepath+filename)
    ...
    best_layers = list(map(int, [X.shape[0], 8, 1]))
    plot_learning_curve_credit_approval(X_train, Y_train, X_test, Y_test, best_layers, best_lambda, learning_rate)