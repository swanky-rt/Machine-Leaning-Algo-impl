...
def load_credit_approval_datasets(filepath):
    data = pd.read_csv(filepath)
    data = data.drop(index=0).reset_index(drop=True)

    label_mapping = {'Cammeo': 0, 'Osmancik': 1}
...
if __name__ == "__main__":
    ...
    data = load_credit_approval_datasets(filepath+filename)
    target_attributes = [col for col in data.columns if col!= 'label']

    evaluate_random_forest(data, target_attributes, 'label', k=10)
