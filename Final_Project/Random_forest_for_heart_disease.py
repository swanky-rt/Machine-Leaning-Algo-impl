...
def load_data_sets(filepath):
    ...
    attributes = df.columns.drop('label').tolist()
...
def evaluate_random_forest(data, attributes, target_variable, k=5, ntree_values=None):
    ...
    for ntree in ntree_values:
        ...
        for fold_index in range(k):
            ...
            precision, f1, recall = calculate_multiclass_performance_metrics(
                test_data[target_variable].values, np.array(test_predictions)
            )
...
def calculate_multiclass_performance_metrics(y_true, y_pred):
...
if __name__ == "__main__":
    ...
    df, attributes = load_data_sets(filepath+filename)

    evaluate_random_forest(df, attributes, 'label', k=10)