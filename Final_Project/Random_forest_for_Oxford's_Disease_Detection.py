...
def get_data_sets(file_name):
    ...
    data_sets.columns = ['MDVP:Fo(Hz)', 'MDVP:Fhi(Hz)', 'MDVP:Flo(Hz)','MDVP:Jitter(%)','MDVP:Jitter(Abs)',
                                    'MDVP:RAP','MDVP:PPQ','Jitter:DDP','MDVP:Shimmer','MDVP:Shimmer(dB)','Shimmer:APQ3','Shimmer:APQ5',
                                    'MDVP:APQ','Shimmer:DDA','NHR',  'HNR', 'RPDE', 'DFA','spread1','spread2','D2', 'PPE', 'Diagnosis']  # target label: 0 = healthy, 1 = Parkinson’s
    data_sets = data_sets.drop(index=0).reset_index(drop=True)
    for col in data_sets.columns[:-1]:  # skip label
        col_max = data_sets[col].max()
        col_min = data_sets[col].min()
        if col_max != col_min:
            data_sets[col] = 2 * (data_sets[col] - col_min) / (col_max - col_min) - 1
        else:
            data_sets[col] = 0
    return data_sets

# def loan_oxford_parkinsons_datasets(filepath):
...
if __name__ == "__main__":
    ...
    data_sets_attributes_for_parkinson=['MDVP:Fo(Hz)', 'MDVP:Fhi(Hz)', 'MDVP:Flo(Hz)','MDVP:Jitter(%)','MDVP:Jitter(Abs)',
                                    'MDVP:RAP','MDVP:PPQ','Jitter:DDP','MDVP:Shimmer','MDVP:Shimmer(dB)','Shimmer:APQ3','Shimmer:APQ5',
                                    'MDVP:APQ','Shimmer:DDA','NHR',  'HNR', 'RPDE', 'DFA','spread1','spread2','D2', 'PPE'  # target label: 0 = healthy, 1 = Parkinson’s
    ]
    data = get_data_sets(filepath+filename)
    test_data = data.head(100) # test with less data
    evaluate_random_forest(data, data_sets_attributes_for_parkinson, 'Diagnosis', k=10)
